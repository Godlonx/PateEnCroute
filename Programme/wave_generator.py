...
class Wave():
    ...
    def bestiare_dispo(self):
        ...
        # print(p.db('Select nom,pwr_lvl,id from Plantes'))
        self.mob_dispo = self.db('Select id_mob from Bestiaire where id_monde like ' + str(self.monde))
        stat_mob = []
        for i in range(len(self.mob_dispo)):
            stat_mob.append(self.db('Select nom,pwr_lvl,id,spawn_rate from Plantes where id like ' + str(self.mob_dispo[i][0])))
